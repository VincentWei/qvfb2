...
class QVFbView : public QScrollView
{
    Q_OBJECT
public:
    QVFbView( int display_id, int ppid, int w, int h, int d, QWidget *parent = 0,
...
private:
	...
	bool imeOpened;
	//bool imeStared;
	//QVFBEventData last_key_data;

private:
	void checkEvents();
	void sendQVFBCloseEvent();
	
	int unpressedKey[50][2];
...
public slots:
	void sendIMEText(const char* szText);
};
